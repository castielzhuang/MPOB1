...
print(la.eq ) #### 最优模型系数结果 为lambda=0.000022

# Lasso筛选变量动态过程图 
la.eq <- glmnet(X, y, family="gaussian", 
                intercept = F, alpha=1) 

tiff(file = "~/Downloads/dataset 20231014/lasso/ffbyield_ plot.tiff",width = 3500, height = 2000, res = 400)
# plot
plot(la.eq,xvar = "lambda", label = F)
dev.off()

la.eq <- glmnet(X, y,lambda=0.000022, family="gaussian", 
                intercept = F, alpha=1) 
cof<-coef(la.eq)
cof
# 也可以用下面的方法绘制
...
tiff(file = "~/Downloads/dataset 20231014/lasso/ffbyield_ plotcvfit.tiff",width = 3500, height = 2000, res = 400)
plot(mod_cv)
dev.off()
